bulle_p={name,tombe,effect,sfxn}

function bulle_p:Init()
	self.name = GetEntityName()
	LoadEntityAnim(self.name,"bulle_p")
...
function bulle_p:Update()
	...
	if (IsEntityInAtomic ("Player1","bulle01","CURRENT") == 1) then
		setShake(1,0.2)
		if Effect == 0 then
...
function bulle_p:End()
end

function bulle_p:Clock(Id)
	
	if Id == 0 then
		StartEntityAnim(self.name,"bulle_p",-1,1,1)
		startSFX(self.sfxn)
	end
