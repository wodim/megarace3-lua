CTRL_choixfonction10={flashing, clock, draw}

-- Function 'Init'
function CTRL_choixfonction10:Init()
	self.flashing = FALSE
...
-- Function 'Update'
function CTRL_choixfonction10:Update()
	...
	if PushEnterSelected()==1 then
		...
		if IsInAcqMode()~=1 then
			...
			self.draw = TRUE
			SetConfigControl(gCurConfig, SWAP_MODULE_CTRL, CTRL_TIME_DELAY)
		end
	end

	local key = GetConfigControl(gCurConfig, SWAP_MODULE_CTRL)
	if key==nil then
		key = GetText(700)
	end
	...
	if IsSelected()==1 then
		PrintTextOn("CTRL_fonction10", 118, S_FONTE, S_LF,"LEFT", S_sel.r1, S_sel.v1,S_sel.b1,S_sel.a1,S_sel.r2,S_sel.v2,S_sel.b2,S_sel.a2)
		...
		PrintTextOn("CTRL_fonction10", 118, S_FONTE, S_LF,"LEFT", S_nsel.r1, S_nsel.v1,S_nsel.b1,S_nsel.a1,S_nsel.r2,S_nsel.v2,S_nsel.b2,S_nsel.a2)
		if self.draw==TRUE then
...
-- Function 'End'
function CTRL_choixfonction10:End()
end
