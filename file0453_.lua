...
-- Function 'Init'
function CAR_BT_CHOIXACTIF01:Init()

end

-- Function 'Update'
function CAR_BT_CHOIXACTIF01:Update()
	...
	if IsSelected()==1 and IsTrackAvailable(gm,trckname) == 1 then
	...
	if PushEnterSelected()==1 then
		...
		elseif IsTrackAvailable(gm,trckname) == 1 then
			JoueSon2DNoInst("MenuVAL", 0,-1)
			JoueSon2D("MenuVAL", "CAT_Fond", 0,-1)
			Selected("CAR_BT_SELECT")
			LoadInterface("Menu_Garage")
--			LanceCourseCarriere(trckidx)
		else
			JoueSon2D("MenuIMP", "CAR_Fond", 0,-1)
		end
	end
end

-- Function 'End'
function CAR_BT_CHOIXACTIF01:End()
