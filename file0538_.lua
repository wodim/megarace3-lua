...
-- Function 'Update'
function NME_6:Update()

	if GetPlatform() == 1 then

		local car=strchar(GetAlphaChar(NMEI6))
		...
		if PushUpSelected() == TRUE then
			...
			NMEI6 = NMEI6 + 1
			if NMEI6 >= GetAlphaSize() then
				NMEI6 = 0
			elseif NMEI6 < 0 then
				NMEI6 = GetAlphaSize() - 1
			end
		end
		...
		if PushRightSelected() == TRUE then
			...
			ShowInterfaceEntity("NME_6")
			HideInterfaceEntity("NME_6_S")
