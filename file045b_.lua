...
-- Function 'Init'
function CAT_BT_CHOIXACTIF01:Init()


end

-- Function 'Update'
function CAT_BT_CHOIXACTIF01:Update()
	...
	if PushRightSelected() == 1 then
		StopRight()
		if GetLuaObject("CAT_Fond").balade == 1 then
	...
	if PushEnterSelected()==1 then
		...
		if IsTrackAvailable(gm,trckname) == 1 then
			...
			Selected("CAT_BT_SELECT")
			LanceCourseCata(trckidx)
		else
			JoueSon2D("MenuIMP", "CAT_Fond", 0,-1)
		end
	end
end

-- Function 'End'
function CAT_BT_CHOIXACTIF01:End()
