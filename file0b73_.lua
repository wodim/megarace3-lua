goutte_h2={name,tb,tomb }

function goutte_h2:Init()

	LoadEntityAnim(GetEntityName(),"goutte_h2")
...
function goutte_h2:Update()
	...
	if (IsEntityInAtomic ("Player1","QuadPatch11","CURRENT") == 1) then
		setShake(1,0.2)
		if self.tb == 0 then
...
function goutte_h2:End()

end

function goutte_h2:Clock(Id)

	if Id == 0 then
		StartEntityAnim(self.name,"goutte_h2",-1,1,1)
	end
end

function goutte_h2:Notify(event)

	if (event == TM_ENTITY_ANIM_FINISH and GetLuaObject("T_K_Yo_fin").pass == 0) then
		JoueSon3DOnAtomic("Gouttes", self.name, "QuadPatch11", 0, -1)
